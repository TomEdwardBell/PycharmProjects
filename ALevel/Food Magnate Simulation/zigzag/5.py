    ...
    def Run(self):
        ...
        while Choice != "Q":
            ...
            if Choice == "1":
                ...
                choice = -1
                while not(0 <= choice < len(self._Companies)):
                    for c in range(len(self._Companies)):
                        print(f'{c + 1}. {self._Companies[c].GetName()}')
                    choice = int(input('\nEnter the number next to the company you wish to modify: ')) - 1

                self.ModifyCompany(choice)
            elif Choice == "4":
                self.AddCompany()
            elif Choice == "6":
                self.ProcessDayEnd()
