...
class Settlement:
    ...
    def RemoveHousehold(self):
        self._Households.remove(random.choice(self._Households))
...
class Simulation:
    ...
    def __ProcessAddRemoveHouseholdsEvent(self):
        if random.random() < 0.3:
            NoOfRemovedHouseholds = random.randint(1, 4)
            for Count in range(1, NoOfRemovedHouseholds + 1):
                self._SimulationSettlement.RemoveHousehold()
            print(str(NoOfRemovedHouseholds) + " households have been removed from the settlement.")
        else:
