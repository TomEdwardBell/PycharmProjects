...
class Simulation:
    ...
    def ModifyCompany(self, Index):
        ...
        if Choice == "2" or Choice == "3":
            ...
            if not (X >= 0 and X < self._SimulationSettlement.GetXSize() and Y >= 0 and Y < self._SimulationSettlement.GetYSize()):
                valid_space_found = False
                print('Invalid coordinates')
            else:
                outlets = []
                for company in self._Companies:
                    outlets += company.GetOutlets()
                for outlet in outlets:
                    if (outlet.GetX(), outlet.GetY()) == (X, Y):
                        valid_space_found = False
                        print('Invalid coordinates. There is an outlet at this position already')

            if valid_space_found:
