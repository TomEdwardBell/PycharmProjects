...
class Location:
    def __init__(self):
        ...
        # Added
        self.Environment = None # By default the Landscape is None
...
class Simulation:
    ...
    def __InputCoordinate(self, CoordinateName):
        ...
        if not -1 < Coordinate < self.__LandscapeSize:
            print(" Not Valid: Coordinate must be between", 0, " and ", self.__LandscapeSize, sep = "")
            Coordinate = self.__InputCoordinate(CoordinateName)
        return Coordinate
    ...
    # added
    def __addroad(self,mode, num):
        if mode == "horizontal":
            row = num
            for i in range(self.__LandscapeSize):
                self.__Landscape[i][row].Environment = "R"
        if mode == "vertical":
            col = num
            for i in range(self.LandscapeSize):
                self.__Landscape[col][i].Environment = "R"
    ...
    def __CreateNewWarren(self):
        ...
        # Added
        while not self.__Landscape[x][y].Warren is None or not self.__Landscape[x][y].Environment is None:
            x = random.randint(0, self.__LandscapeSize - 1)
            y = random.randint(0, self.__LandscapeSize - 1)
        print(self.__Landscape[x][y].Environment)
        if self.__ShowDetail:
    ...
    def __CreateNewFox(self):
        ...
        y = random.randint(0, self.__LandscapeSize - 1)
        # Added
        while not self.__Landscape[x][y].Fox is None or not self.__Landscape[x][y].Environment is None:
            x = random.randint(0, self.__LandscapeSize - 1)
            y = random.randint(0, self.__LandscapeSize - 1)
        if self.__ShowDetail:
        ...
        for FoxX in range(0, self.__LandscapeSize):
            for FoxY in range (0, self.__LandscapeSize):
                if not self.__Landscape[FoxX][FoxY].Fox is None:
                    ...
                    # Added
                    if self.__CheckEnvironmentBetween(FoxX, FoxY, WarrenX, WarrenY):
                        if Dist <= 3.5:
                            PercentToEat = 20
                        elif Dist <= 7:
                            PercentToEat = 10
    ...
    # Added
    def __CheckEnvironmentBetween(self, x1, y1, x2, y2):
        clear = True
        y = y1
        for x in range(x1, x2 + 1):
            print("(",x,",",y,"): ",self.__Landscape[x][y].Environment, sep = "")
            if not self.__Landscape[x][y].Environment is None:
                clear = False
    ...
    def __DrawLandscape(self):
        ...
        for y in range (0, self.__LandscapeSize):
            ...
            for x in range (0, self.__LandscapeSize):
                # Added
                if not self.__Landscape[x][y].Environment is None:
                    print(" ", self.__Landscape[x][y].Environment, " ", sep="", end = "")
                # Added
                else:
                    if not self.__Landscape[x][y].Warren is None:
